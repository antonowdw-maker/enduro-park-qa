...
test.describe('Sort & pagination UI', () => {
  ...
  test('TC-SORT-02: клик Год — asc, повторный — desc', async ({ page }) => {
    ...
    expect.soft(isSortedAscNumbers(yearsAsc)).toBeTruthy();
    ...
    expect.soft(isSortedDescNumbers(yearsDesc)).toBeTruthy();
    expect.soft(yearsDesc[0]).toBeGreaterThanOrEqual(2026);
  });
  ...
  test('TC-PAGINATION-01: limit 10 → 10 строк, next меняет страницу', async ({ page }) => {
    ...
    // После open() лимит уже 10 — selectOption('10') не шлёт новый GET
