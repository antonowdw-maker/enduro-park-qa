...
**Полный учебный контракт:** [`KNOWN-BUGS.md`](KNOWN-BUGS.md) (цель обучения, owner, `test.fail` / `KNOWN_BUGS_MODE`, API-границы).  
**Не считать** эти дефекты «production-ready».

| ID | Описание | Фактическое поведение | Тест |
|----|----------|----------------------|------|
| BUG-01 | Фильтр «Ремонт» ≠ «В ремонте» в таблице | Расхождение текста | TC-FILTER-*; `known-bugs.spec.ts` |
| BUG-02 | Марка `TEST` + модель `123` | UI `form-server-error` / API **403** про guest | TC-BIKE-NEG-10; UI+API known-bugs |
| BUG-03 | Границы **года выпуска** | Ошибка на **1989** и **(текущий+1)**; **1988** и **(текущий+2)** проходят. Текст верхней ошибки: «Год не может быть позже **{текущий год}**» (не `{текущий+1}`) | TC-BIKE-NEG-06…09; `known-bugs-api.spec.ts` |
