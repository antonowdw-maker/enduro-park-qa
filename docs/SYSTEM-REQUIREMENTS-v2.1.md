...
|----------|----------------------------|
| `limit` | `Number` → иначе **10**; затем clamp **1…50** (`0`/текст → 10; отрицательное → 1; `>50` → 50) |
| `offset` | если передан непустой — `max(0, Number \|\| 0)`; иначе из `page` |
| `page` | только без `offset`: `max(1, Number \|\| 1)`; `offset` имеет **приоритет** |
