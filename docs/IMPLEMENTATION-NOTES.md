...
| G | Security hardening | 🔄 `feature/qa-wave-g-security` (slice1+2: perimeter, CSRF, Zod) |
| H | Known-bugs contract | ⏳ |
| I | Coverage matrix + CI gates | ⏳ |
| J | Rental epic | ⏳ |

Рекомендуемый старт после мержа **G**: **H…**.
...
*Последнее обновление: 15.07.2026 — G slice2 + UI CSRF на `feature/qa-wave-g-security`.*
