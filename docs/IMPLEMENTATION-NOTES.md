...
## Статус сессии 13.07.2026 (вечер) — точка остановки

**Ветка:** `feature/backlog-improvements`  
**Последний коммит п.3:** дата ТО — текст + календарь + маска

### Сделано сегодня (бэклог)

| # | Задача | Статус | Коммиты / заметки |
|---|--------|--------|-------------------|
| 1 | `pagination-limit` под таблицей | ✅ | + fix битой cookie на GET /bikes |
| 2 | Мультивыбор статусов | ✅ | API `status=a,b`; TC-FILTER-MULTI-* |
| 3 | Дата ТО: текст + календарь + маска | ✅ | `input-lastService` + `input-lastService-calendar`; TC-BIKE-LAST-* |

Требования и ТК по п.2–п.3 **уже обновлены** в `SYSTEM-REQUIREMENTS-v2.1.md` и `MANUAL-TEST-CASES-v1.2.md`.

### С чего начать завтра

1. **П.4** — вынести из `MainPage.tsx` компоненты `BikeTable` и `BikeFilters` (рефакторинг, поведение без изменений).
2. Затем по порядку: **п.5** (`useCallback` / deps `loadData`) → **п.6** (`authService.ts`) → **п.7** (корневой `package-lock.json`).
3. После закрытия бэклога — merge в `main`, потом **итерация 10** (Playwright).
...
*Последнее обновление: 13.07.2026 — стоп после бэклог п.3; старт завтра с п.4.*
