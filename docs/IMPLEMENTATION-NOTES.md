...
| Фильтр марка/модель | Только общий `search` | Явные UI + API `brand`/`model` (подстрока LIKE) | ✅ Бэклог product | F-FILTER-11…14 |
...
- ~~Фильтр по марке / модели (UI + API + ТК + E2E).~~ → ветка `feature/filter-brand-model` (F-FILTER-11…14).
