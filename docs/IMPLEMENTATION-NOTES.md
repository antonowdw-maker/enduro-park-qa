...
| F | a11y / mobile | ✅ `feature/qa-wave-f-a11y-mobile` |
| G | Security hardening | ⏳ |
| H | Known-bugs contract | ⏳ |
| I | Coverage matrix + CI gates | ⏳ |
| J | Rental epic | ⏳ |

Рекомендуемый старт: **G → H…** (A–F закрыты на ветках / в `main` по мере merge).
...
| — | Hotfix QA: wrap статусов, сетка полей фильтра, модалка `100dvh`, mobile-sort | ✅ после ручной проверки |

#### Тимлид QA automation — **APPROVE WITH NOTES**

| Sev | Finding | Решение |
|-----|---------|---------|
| — | `a11y-mobile.spec.ts`: TC-A11Y-01…05 + TC-MOBILE-01…04; PO: `bikeCard`, visible filter на edit/delete | ✅ |
| — | Delete Enter на уникальном VIN (`buildUniqueVin`), не ломает seed-якоря | ✅ ок |
| — | Smoke CRUD после dual-DOM — 6/6 (регресс постов/кликов) | ✅ ок |
| — | Locators: `filter({ visible: true })` для row/card dual mount; `expectTableHasRows` не цепляет скрытую карточку | ✅ ок |
| P2 | Focus-trap: assert через `activeElement.closest(modal)`, не axe/axe-core | достаточно волны F; axe → I |
| P3 | Нет отдельного E2E на Shift+Tab wrap и restore focus после Escape | nice; не блокер |

Вердикты: **fullstack APPROVE WITH NOTES**, **QA automation APPROVE WITH NOTES**. Коммит — по апруву.
...
*Последнее обновление: 15.07.2026 — волна F (a11y + mobile cards).*
