...
Источник истины по **поведению** — код + актуальные версии в `docs/` (SYSTEM / MANUAL).
**Витрина:** [`README.md`](../README.md). **Летопись этапов:** [`PROJECT-HISTORY.md`](PROJECT-HISTORY.md).
...
| **Идеи «сделать позже»** | В [`QA-STAND-ROADMAP.md`](QA-STAND-ROADMAP.md) или новую волну — не раздувать README чеклистами. |
| **Перед merge в `main`** | Просмотр таблиц: что перенести в SYSTEM-REQUIREMENTS. |
| **После волны Playwright** | В MANUAL пометить ТК: `🤖 Автотест:`; обновить таблицу TC-* ниже. |
...
| G | Security hardening | ✅ `main` (15.07.2026) |
| H | Known-bugs contract | ✅ `main` (15.07.2026) |
| I | Coverage matrix + CI gates | ✅ `main` (15.07.2026) |
| J | Rental epic | ⏸ вне EPM → репо `willim-rental` |
...
| Sev | Finding |
|-----|---------|
| P1 | `MainPage` search debounce: безусловный `setPage(1)` через 300ms на mount сбрасывал пагинацию (CI chromium `TC-PAGINATION-02`) |
| — | Фикс: `setPage(1)` только при реальной смене `debouncedSearch` |

#### Тимлид QA automation — **APPROVE**
| Sev | Finding |
|-----|---------|
| — | Known-bugs ✘ в логе — teaching mode; единственный job-fail — `TC-PAGINATION-02` |
| — | E2E: prev снова через `runAndWaitForBikes` + полный индикатор страницы |

### Ревью тимлидов — hotfix nightly firefox/webkit flake (17.07.2026)

#### Тимлид fullstack — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | Продукт не менялся; стабилизация E2E через `tap`, UI-wait на clear/toggle, `setLimit50` poll на alt-браузерах |
| P3 | `page.waitForTimeout` в `postActionDelayMs` — только опция; в hotfix не используется в prod-path |

#### Тимлид QA automation — **APPROVE**
| Sev | Finding |
|-----|---------|
| — | Корень: `waitForResponse` + strict query на clear/toggle в firefox/webkit (nightly 17.07) |
| — | Локально: firefox 57/57 + webkit 57/57 на затронутых specs; Promise.all в `waitForBikesApi` |
| P3 | Полный `npm test` nightly-matrix — в CI; локально прогнаны кластеры filters/search/seed/pagination/xss |

### Ревью тимлидов — закрытие EPM / вынос J (15.07.2026)

#### Тимлид fullstack — **APPROVE**
| Sev | Finding |
|-----|---------|
| — | README + roadmap: A–I done, J → `willim-rental`; docs/rental в EPM не коммитились (черновик убран локально) |
| — | node_modules не в коммите |

#### Тимлид QA automation — **APPROVE**
| Sev | Finding |
|-----|---------|
| — | Портфолио-стенд заморожен на A–I; coverage/CI/known-bugs без регрессии от docs-only |
| — | Новый продукт — отдельный workspace/чат, без смешения с E2E EPM |

### Ревью тимлидов — волна I (15.07.2026)

#### Тимлид fullstack — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | `typecheck` scripts + CI `quality` before Playwright — правильный gate |
| — | Фиксы под lint/tsc (MainPage coerce ranges, filterValidation extract, useModalA11y effect) |
| P3 | frontend lint всё ещё warning на RHF `watch` (incompatible-library) — не блокирует exit 0 |
| P3 | npm audit в CI сознательно не добавляли |

#### Тимлид QA automation — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | `COVERAGE-MATRIX.md` F↔TC↔spec; gap TC-AUTH-05 зафиксирован |
| — | Артефакты именованы единообразно (chromium / nightly-browser) |
| P2 | Матрица укрупнённая (группы F), не каждая строка MANUAL — достаточно для I; детализация → по мере нужды |
...
### Ревью тимлидов — волна G slice1 (15.07.2026)

#### Тимлид fullstack — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | Helmet + явный JSON 100kb + `/health`/`/ready` — чистый периметр |
| — | Rate-limit default ON; CI/local: `DISABLE_LOGIN_RATE_LIMIT` (+ legacy ENABLE=false) |
| P3 | Отдельный `PrismaClient` в `server.ts` для `/ready` — не shared singleton с репозиториями; ок для стенда |
| P3 | CSRF / Zod — осознанно вне slice1 |

#### Тимлид QA automation — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | `security-perimeter-api.spec.ts` + `security-xss.spec.ts` green |
| — | XSS assert: экранированный HTML (`&lt;script&gt;`), не живые теги |
| — | Docs: SECURITY / SYSTEM 2.11 / MANUAL 1.12 / e2e README |
| P2 | Rate-limit default-on не гоняется в PR CI (DISABLE); isolated opt-in остаётся — достаточно |
| P3 | Нет assert на полный набор Helmet-заголовков — хватит nosniff |

### Ревью тимлидов — волна G slice2 (15.07.2026)

#### Тимлид fullstack — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | CSRF double-submit: `GET /api/auth/csrf`, header `X-CSRF-Token`, login без CSRF |
| — | Zod DTO login + bike (`.strip()`); BUG-03 year сохранён; Axios xsrf + `ensureCsrf` |
| P3 | Дублирование year/VIN правил: Zod DTO + `bikeService.validate*` — приемлемо для стенда |
| P3 | `DISABLE_CSRF=true` аварийный bypass — задокументирован, не в CI |

#### Тимлид QA automation — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | `security-csrf-zod-api.spec.ts` TC-SEC-CSRF/ZOD; `loginAuth` в CRUD/roles/perimeter |
| — | Anon-мутации: CSRF first → 401 (не 403); lifecycle logout с CSRF |
| — | Docs: SECURITY / SYSTEM 2.12 / MANUAL 1.13 / roadmap G закрыт |
| P2 | Нет отдельного UI E2E «logout после CSRF» — покрыто Axios interceptor + auth smoke/XSS CRUD | ✅ закрыто: `security-csrf-ui.spec.ts` TC-SEC-CSRF-UI-01/02 |
| P3 | Rate-limit по-прежнему isolated; CSRF всегда on в CI — ок |

Вердикты: **fullstack APPROVE WITH NOTES**, **QA automation APPROVE WITH NOTES**.

### Ревью тимлидов — G CSRF UI hotfix (15.07.2026)

#### Тимлид fullstack — **APPROVE**
| Sev | Finding |
|-----|---------|
| — | Без product-изменений; UI тесты только наблюдают Axios headers |

#### Тимлид QA automation — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | TC-SEC-CSRF-UI-01/02: cookie `csrf` + header на logout/create |
| — | MANUAL 1.14 / e2e README / SYSTEM F-AUTH-09 обновлены |
| P3 | Create UI дублирует CRUD smoke — осознанно ради assert заголовка |
...
BUG-01, BUG-02, BUG-03 — см. [`KNOWN-BUGS.md`](KNOWN-BUGS.md) / требования §7 / ручные ТК.  
В автотестах — правильные ожидания + `markExpectedFailure()` (`KNOWN_BUGS_MODE=teaching` по умолчанию).

### Ревью тимлидов — волна H (15.07.2026)

#### Тимлид fullstack — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | Каталог KNOWN-BUGS без правок product-багов — корректный scope H |
| — | Режимы teaching/strict через env, без runtime toggle в бэке — достаточно для стенда |
| P3 | Факт-тесты API создают байки с year 1988/current+2 — засорение БД в длинном suite; seed в globalSetup ок |

#### Тимлид QA automation — **APPROVE WITH NOTES**
| Sev | Finding |
|-----|---------|
| — | `known-bugs-api.spec.ts`: матрица 1988/1989/+1/+2 + BUG-02 FACT/TARGET |
| — | UI переведён на `markExpectedFailure`; MANUAL 1.15 / SYSTEM 2.13 |
| P2 | Нет отдельного CI job `KNOWN_BUGS_MODE=strict` (ожидаемо красный) — документировано, opt-in |

---

*Последнее обновление: 15.07.2026 — I влита в `main`; далее J.*
