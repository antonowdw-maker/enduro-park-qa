...
Открытых идей из этого списка нет. Новые — добавлять в README → отдельная feature-ветка от `main`.
...
*Последнее обновление: 14.07.2026 — бэклог «на подумать» закрыт (фильтр, seed, nightly) в `main`.*
