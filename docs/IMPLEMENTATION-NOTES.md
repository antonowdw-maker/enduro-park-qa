...
| G | Security hardening | 🔄 `feature/qa-wave-g-security` (slice1 perimeter) |
| H | Known-bugs contract | ⏳ |
| I | Coverage matrix + CI gates | ⏳ |
| J | Rental epic | ⏳ |

Рекомендуемый старт: добить **G** (CSRF, Zod/DTO), затем **H…** (A–F в `main`).
...
*Последнее обновление: 15.07.2026 — F в `main`; G slice1 (perimeter) готов к коммиту.*
