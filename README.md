...
- [ ] **Мультивыбор статусов в фильтрах** — выбирать несколько статусов одновременно (напр. «Доступен» + «В ремонте»).
- [ ] Поле «Последнее ТО»: ввод даты вручную (текстом), а не только через календарь `type="date"`.

### Рефакторинг фронтенда
- [ ] Доработать `MainPage.tsx`: вынести `BikeTable` и `BikeFilters` в отдельные компоненты (частично сделано: `BikeFormModal`).
